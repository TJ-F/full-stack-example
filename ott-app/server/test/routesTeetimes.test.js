...
describe('Teetimes Routes', () => {
    ...
    const mockDb = {
        query: async (q, params) => {
            if (q.includes('FROM teetime WHERE DATE(date_time) =')) {
                return [teetimes];
            }

            if (q.includes('FROM teetime WHERE DATE_FORMAT')) {
            ...
            if (q.includes('SELECT * FROM teetime')) {
                return [teetimes];
            }

            if (q.startsWith('INSERT INTO teetime')) {
                return [{ insertId: 99 }];
            }
    ...
    it('GET /available-teetimes should return tee times with availability', async () => {
        ...
        assert.strictEqual(res.status, 200);
        assert.strictEqual(Array.isArray(res.body), true);
        // console.log("Available tee times:", res.body);
    });
    ...
    it('POST /reserved-teetimes should fail without token', async () => {
        ...
        assert.strictEqual(res.body.message, 'No token provided or invalid format');
    });
